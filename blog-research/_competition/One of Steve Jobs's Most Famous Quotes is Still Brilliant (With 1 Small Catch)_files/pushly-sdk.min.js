...
try {
    window.PushlySDK.watch("ready", () => {
        let parselyData = document.cookie.match(/_parsely_visitor=([^;]*)/);
        if (parselyData) {
            parselyData = JSON.parse(decodeURIComponent(parselyData[1]));
            if (parselyData?.id) { window.PushlySDK.push(["external_id", parselyData.id]);}
        }
    });
} catch (e) {}
